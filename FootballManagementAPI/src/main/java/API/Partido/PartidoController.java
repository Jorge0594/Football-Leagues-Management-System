...
public class PartidoController {
	...
	public interface PartidoView extends Estadio.BasicoAtt, Estadio.DatosAtt, Partido.InfoAtt, Partido.RestAtt, Jugador.EquipoAtt, Jugador.PerfilAtt, Equipo.RankAtt, Sancion.JugadorAtt, Sancion.SancionAtt, VistaGrupo.VistaGrupoAtt, VistaTemporada.VistaTemporadaAtt, Incidencia.IncidenciaAtt {
	}
	...
	public ResponseEntity<Partido> modificarPartido(@PathVariable String id, @RequestBody Partido partido) {
		...
		// Liga ligaDelPartido =
		// ligaRepository.findByNombreIgnoreCase(partido.getLiga());
		if (entrada == null) {
			return new ResponseEntity<Partido>(HttpStatus.NOT_FOUND);
		} else {
			/*
	...
	public ResponseEntity<Partido> eliminarPartidoId(@PathVariable String id) {
		...
		// Liga ligaDelPartido =
		// ligaRepository.findByNombreIgnoreCase(entrada.getLiga());
		if (arbitroDelPartido != null) {
		...
		/*
		 * if (ligaDelPartido != null) { // Elimina el partido de su liga. if
		 * (!ligaDelPartido.getPartidos().isEmpty()) {
